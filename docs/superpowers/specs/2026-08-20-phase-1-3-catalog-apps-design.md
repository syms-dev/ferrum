...
**A stricter requirement this implies, found while mapping the implementation:** `settings.json` is read via `builtins.readFile` at Nix *evaluation* time (`ferrum.lib.mkHost`'s `settings` argument) — meaning any value from it that gets Nix-string-interpolated into a systemd unit's `ExecStart`/`script`/`environment` is compiled into the unit file, which lands in `/nix/store`. The Nix store is **world-readable by default**, to every local user, not just root. Naively writing `${app.settings.vpnWireguardConfig}` into the netns-setup unit's script would therefore expose the WireGuard private key more broadly than `settings.json` itself (a root-owned file) already does — a strictly worse outcome than the accepted tech debt, not an equal one.
