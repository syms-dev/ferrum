...
Returns the generation list the rollback UI is built on. The handler **embeds the full `ferrum_state::journal::JournalEntry`** under `snapshot` — all five of its real fields (`snapshot`, `generation`, `toplevel`, `taken_at`, `quiesced`, per `crates/ferrum-state/src/journal.rs:4-14`) — rather than constructing a trimmed DTO. One serialised shape for a journal entry means there is no second struct to keep in step with the crate, and `toplevel` is genuinely useful to an operator comparing what a generation was actually running.

```json
{ "current": 42,
  "generations": [
    { "generation": 42, "date": "1773001331", "current": true,
      "snapshot": { "snapshot": "1773001331-gen42", "generation": 42,
                    "toplevel": "/nix/store/...-nixos-system-ferrum",
                    "taken_at": "1773001331", "quiesced": true },
      "rollbackable": false,
      "reason": "generation 42 is the generation this host is already running -- rolling back to it would restore every app's state directory from the last apply's snapshot and reboot, without changing the system closure" },
    { "generation": 41, "date": "1772994127", "current": false, "snapshot": null,
      "rollbackable": false,
      "reason": "generation 41 has no state snapshot -- it was either applied outside ferrum-apply, or its snapshot was pruned" }
  ] }
```

For a generation that is **not** the current one, `rollbackable`/`reason` come from `generations::is_rollbackable`'s existing `Result<(), String>` — the UI shows the crate's own message and offers no rollback control, rather than inventing a second vocabulary for why a generation cannot be rolled back. This is the same discipline the 1.5a spec applied to `ApplyResult`.

**The currently-running generation is always reported `rollbackable: false`.** This was put to the product owner as an open acceptance question and answered: block it. Rolling back to the generation already running is a destructive no-op — it reverts every app's state directory to the last apply's snapshot and reboots the host, while changing nothing about the system closure — and without this rule it is offered on the most innocuous-looking row in the list, the one the operator is least likely to read a warning on. No confirmation-dialog wording is specified for this case because the affordance is not offered at all: the row shows the reason and no control.

**Where that check lives matters.** `ferrum_state::generations::is_rollbackable` tests snapshot **presence** only (`generations.rs:73-81`) and has no notion of which generation is current; `ferrum-apply`'s own `rollback::prepare` (`rollback.rs:10-49`) likewise has no `target_generation == current` guard. The current-generation check is therefore **ferrumd's own, layered on top of the crate's result** — do not edit the shared crate for it, and do not reuse the crate's message. `gc.rs:76` protects the current generation's snapshot by design, so the current generation reliably *does* have a snapshot: this is the normal case, not an edge case, and `is_rollbackable`'s "no state snapshot ... or its snapshot was pruned" wording would be actively false here. The reason string must say the generation is **already running**.

`rollbackable: true` is a **precheck, not a guarantee.** `gc.rs:105-132` deletes a snapshot subvolume *before* its journal entry, so a crash between the two leaves an entry pointing at nothing; `rollback.rs:43-49` catches exactly that and fails loudly. `POST /api/jobs` remains the authority on whether a rollback can actually proceed.

**Error behaviour.** An unset `FERRUM_PROFILES_DIR` or `FERRUM_JOURNAL_DIR`, an unreadable or absent profile directory, or a `system` symlink that cannot be resolved each produce a **`500` naming the real path**, following the pattern `crates/ferrumd/src/catalog.rs:12-26` already ships. A fault must never render as an empty list, because an empty list is indistinguishable from the valid "this host has never applied through ferrum-apply" state. `current: null` is reserved for the one narrow case where `system` resolves to a target that does not match `system-<N>-link` — the list itself is still truthful there. A **missing journal directory is not an error**: `crates/ferrum-state/src/journal.rs:36-39` returns `Ok(vec![])` for it, and the correct rendering is "every generation exists, none is rollbackable."
...
**`date` is unix-seconds-as-a-string, and the UI localises it.** Each generation's timestamp comes from its own link's `lstat` mtime and is serialised in exactly the shape `ferrum_state::journal::JournalEntry::taken_at` already uses (`crates/ferrum-state/src/journal.rs:12`) — the same decision `crates/ferrum-apply/src/apply.rs:339-345` records for `chrono_taken_at()`, whose own comment says it "deliberately avoids pulling in the `chrono` crate for one call site." **No third-party dependency is added for this**, and `crates/ferrumd/Cargo.toml` gains only the `ferrum-state` path dependency.

The wire value is therefore a **timezone-free epoch**, and rendering it in the operator's own locale is the browser's job. Server-side local time was considered and rejected: `nix-env --list-generations` renders local time, but no `TZ` or `time.timeZone` is set anywhere in `modules/`, so there is no host timezone to render *to*; and the usual library route for "now, locally" returns an indeterminate-offset error inside a multi-threaded (tokio) process. An epoch on the wire has no such ambiguity to get wrong.
...
`ferrum.storage.journalDir` (default `/var/lib/ferrum/journal`) has no `systemd.tmpfiles` rule today — it is created by `journal::write`'s own `create_dir_all` (`crates/ferrum-state/src/journal.rs:21`) running as root, landing at whatever the ambient umask gives, in practice `0755 root:root`. ferrumd can therefore already read it: `modules/core/storage.nix:57` grants the `ferrum` group r-x on `/var/lib/ferrum`, and the journal directory itself is **world**-readable. That is the problem, not the solution. This phase adds an explicit rule alongside the existing ones in `modules/core/storage.nix`:

```
"d ${cfg.journalDir} 0750 root ${ferrumdGroup} - -"
```

reusing that file's existing `ferrumdGroup` binding, which already falls back to `root` on a host with `ferrum.daemon.enable = false` precisely so naming a nonexistent group cannot fail tmpfiles at boot. The rule **tightens 0755 → 0750**: rollback bookkeeping stops being world-readable, and the daemon's read access becomes stated rather than inherited from a umask. It enables nothing that did not already work.

Because `journalDir` is an operator-settable `types.str` with no constraint today (`modules/core/options.nix:39-43`), this phase also adds a NixOS assertion so the new rule cannot collide with an existing one — see the implementation plan's Task 3.
...
4. **Generations** — the list from `GET /api/generations`, rendering each `date` in the operator's own locale from the epoch the API returns. Rollback is gated behind a confirmation dialog that names the target generation, its snapshot's `taken_at`, and — per the Global Constraint above — states concretely and in the operator's own terms what will revert (the system closure and every app's state directory) and what will not (media files, in-flight downloads, ACME certificates, Authelia users and their password changes). Generations with `rollbackable: false` show the daemon's own `reason` and no control — which includes the currently-running generation, so no dialog is ever reachable for it.
...
- **Unit tests** for: profile-directory parsing (including a directory containing a `system` symlink and non-`system-N-link` entries), the `rollbackable`/`reason` mapping including the current generation's block, the generations endpoint's error behaviour, the SPA fallback's `/api/` exclusion, path-traversal rejection, and the job-list query's `limit` clamping.
...
3. **Reading `/nix/var/nix/profiles/` directly is a second reader of a format `nix-env` owns.** It is a stable, long-documented layout, but it is not an API contract. **COST-02, accepted:** this risk's original mitigation rested on both readers of this directory emitting one shared date format. That is no longer true — ferrumd emits a timezone-free unix epoch while `parse_nix_env_list` parses `nix-env`'s rendered local-time string — so ferrumd's format is now independent of `nix-env`'s and that half of the mitigation is gone. What remains is that `parse_nix_env_list` survives with its tests as the fallback path if the direct read ever proves wrong. Accepting role: **Technical Architect**. Revisit trigger: a NixOS release note changes the `/nix/var/nix/profiles/` layout, or Task 8's VM test's generation assertion fails → switch to `parse_nix_env_list`.
