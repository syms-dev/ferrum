...
**Scope boundary, matching the original design doc's own risk list:** app-to-app registration only. No quality-profile sync, no Recyclarr integration — declarative *arr configuration is bottomless, and Phase 1 holds the line here on purpose.
...
2. **sops-nix's private age key's own lifecycle.** Where does the box's private key live, how is it provisioned during `nixos-anywhere` install, and what happens on a full box replacement? Not fully specified here — needs answering during the implementation plan, since it's the one piece of this whole mechanism that, if lost, makes every existing secret permanently unrecoverable.
